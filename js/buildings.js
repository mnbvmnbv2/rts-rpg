...
class BuildingClass { //felles class for fiende og player 
...
function addBuildingButtons(){
    ...
    //addBtn("Mine")
    for(i in buildings){ //for i in buildings
        addBtn("Choose " + buildings[i].name + rarityStars(buildings[i].rarity),
        //textBox
        "<font color='gold'>"+buildings[i].gold+" gold</font>"+ //Prisen
        "<br><font color='#66FCF1'>"+buildings[i].name+"</font>"+ //navnet
        "<br>"+rarityStars(buildings[i].rarity)+ //hvor sjelden utifra stjerner etc
        "<br>Level: "+buildings[i].level+ //lvl
        "<br>Health: "+buildings[i].health+ //liv
        "<br>AttackPower: "+buildings[i].damageMin+"-"+buildings[i].damageMax+ //hvor mye skade
        "<br>AttackSpeed: "+buildings[i].atSpeed+
        "<br>Experience: "+buildings[i].xp+"/"+buildings[i].maxXp, //hvor rask
